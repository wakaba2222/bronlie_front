...
//				$result[] = $cnt."行目　商品コード: ".$c['product_code']." 在庫更新しました。 在庫数（".$ret[0]['cnt']."）";
				...
				if ($ret[0]['cnt'] == '1')
				{
					...
					$ret = DB::update('dtb_product_sku')->join('dtb_products')->on('dtb_product_sku.dtb_products_product_id', '=', 'dtb_products.product_id')->value('stock', $stock)->value('update_date', date('Y-m-d H:i:s'))->where('product_code', $c['product_code'])->where('del_flg', 0)->where('shop_id', $c['shop_id'])->execute();
...
//print("</pre>");
//exit;
		$keys = array('name', 'name_en', 'name_kana', 'info', 'comment', 'status', 'del_flg', 'create_date', 'view_date', 'close_date', 'product_status_id', 'group_code', 'shop_id', 'brand_id', 'category_id', 'theme_id', 'point_rate', 'update_date', 'season', 'material', 'size_text', 'remarks', 'country','product_id', 'switch_property');
		if (count($csv_key) != count($keys))
		{
// 			$this->arrResult['error'] = "項目数に違いがあります。".count($keys).'の項目が必要です。';
// 			$this->tpl = 'smarty/admin/product/upload.tpl';
