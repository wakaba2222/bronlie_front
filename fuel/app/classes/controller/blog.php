...
//			var_dump($arrShopUser);
		...
		if( $post_id != 0 ) {
			...
			$data2 = get_br_post_detail($post_id);
			...
			for($i = 0;$i < $cnt;$i++)
			{
				$e_key = 'blog要素_'.$i.'_blog_editor';
				$p_key = 'blog要素_'.$i.'_product_id';
				...
				foreach($data2 as $d)
				{
					...
					{
						if ($d['meta_value'] != '')
							$detail_data[] = $d['meta_value'];
					}
					else if ($d['meta_key'] == $p_key)
					{
						if ($d['meta_value'] != '')
						{
							$p = Tag_Item::get_detail($d['meta_value'], false);
							if ($p == null)
								continue;
							if (isset($p['brand_name']))
								...
								$product['brand_name'] = '';
							if (isset($p['price01']))
								$product['price01'] = $p['price01'];
							else
								$product['price01'] = '';
							$product['shop_name'] = $p['shop_name'];
							$product['shop_id'] = $p['login_id'];
							$product['product_id'] = $p['product_id'];
							$p_img = Tag_Item::get_detail_images($d['meta_value']);

							$product['image'] = $p_img[0]['path'];
...
//var_dump($arrResult['arrData']);

			$arrTemp = array();

			foreach($arrResult['arrData']['arrPosts'] as $article)
			{
				if ($article['pin'] != '')
				{
					$article['pin'] = 'on';
					$article['thumb_url'] = str_replace('origin-develop.bronline.jp', 'bronline3.wdemo.org', $article['thumb_url']);
					$arrTemp[] = $article;
				}
			}
			foreach($arrResult['arrData']['arrPosts'] as $article)
			...
			$arrResult['arrData']['arrPosts'] = $arrTemp;
...
$this->debug(' END 処理時間 = ' . $sec . ' 秒');
	...
	{	
		$fp = fopen("/var/www/bronline/fuel/app/logs/blog".date('Ymd').".log", "a");
		if ($fp)
		{
