...
*/
		...
		$arrResult['jcode'] = ( $cart_ctrl->cart->getPaymentType() == "1" || $cart_ctrl->cart->getPaymentType() == "5" ) ? "shift_jis" : "";
...
//exit;
$this->debug("confirm.");
//$this->debug(print_r($arrResult,true));
	...
	public function action_cancel()
	{
		...
		$tpl = 'smarty/cart/cancel.tpl';

$this->debug("sbpayment cancel.");
		return View_Smarty::forge( $tpl, $arrResult, false );
	}
	...
	public function action_error()
	{
		...
		$tpl = 'smarty/cart/error.tpl';

$this->debug("sbpayment error.");
	return View_Smarty::forge( $tpl, $arrResult, false );
	}
	...
	public function action_response()
	{
		...
		$tpl = 'smarty/cart/response.tpl';

		if ($_REQUEST['res_result'] == 'OK')
			print("OK,");
		else
		{
			print("NG,".$_REQUEST['res_err_code']);
		}
$this->debug("response.");
...
//$this->debug(print_r($_SESSION,true));
		...
		if (count($_POST) > 0 && !isset($_POST['ErrCode']) && !isset($_POST['OrderID']) && !isset($_POST['res_result']))
		{
			$cart_ctrl->getSession();
 			if (count($cart_ctrl->cart->getOrderDetail()) == 0)
 			{
		...
		else if (count($_POST) > 0 && isset($_POST['res_result']))
		{
			if (isset($_POST['order_id']) && $_POST['res_result'] == 'OK')
			{
$this->debug("ok.");
				$arrRet2 = DB::select()->from('dtb_order')->where('order_id',$_POST['order_id'])->execute()->as_array();
				if (count($arrRet2) > 0)
				{
					$arrResult['msg1'] = "購入完了しています。";
					return View_Smarty::forge( $tpl, $arrResult, false );
				...
				$arrRet = DB::select()->from('dtb_order_temp')->where('order_id',$_POST['order_id'])->execute()->as_array();

				if (count($arrRet) > 0)
				{
$this->debug("session repare.");
					Log::info('セッションの復帰');
...
//print("<br>");
$this->debug("set order.");

			if( Input::post('PayType') != null || Input::post('pay_method') != null) {
				$res = array();
//				$res = $pay->gmoResponseCheck( Input::post() );
				$res['order_id'] = Input::post('order_id');
				$res['pay_type'] = Input::post('pay_method');
				$res['error'] = false;
				if (Input::post('res_result') != "OK")
				{
					$res['error'] = true;
					$res['error_id'] = Input::post('res_err_code');
					$res['error_msg'] = Input::post('res_payinfo_key');
				}
//var_dump($res);
				// エラーが返却された場合はエラーメッセージを表示させる
				if ($res['error'] == true) {
...
$this->debug("set order complete. ");
//$this->debug(print_r($cart_ctrl->cart,true));
		if (isset($_POST['ErrCode']) && $_POST['ErrCode'] == "")
		{
			...
			{
				$arrOrder = DB::select()->from('dtb_order_detail')->where('order_id',$_POST['OrderID'])->execute()->as_array();
				Controller_Api::send_order($arrOrder);
$this->debug("set gentedimare ".$_POST['OrderID']);
			}
			else if (isset($_POST['order_id']) && $_POST['order_id'] != '')
			{
				$arrOrder = DB::select()->from('dtb_order_detail')->where('order_id',$_POST['order_id'])->execute()->as_array();
				Controller_Api::send_order($arrOrder);
$this->debug("set gentedimare ".$_POST['order_id']);
			}
		}
...
$this->debug("set gentedimare ".$_POST['order_id']);
			...
			{
//$this->debug(print_r($cart_ctrl->cart, true));
$this->debug("set gentedimare ".$cart_ctrl->cart->getOrderId());
				$arrOrder = DB::select()->from('dtb_order_detail')->where('order_id',$cart_ctrl->cart->getOrderId())->execute()->as_array();
				Controller_Api::send_order($arrOrder);
			}
		}
		else
		{
$this->debug("order fail.");
...
// 			exit;
	...
	{	
		$fp = fopen("/var/www/bronline/fuel/app/logs/cart".date('Ymd').".log", "a");
		if ($fp)
		{
