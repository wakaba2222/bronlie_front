...
$this->debug(' ('.$product_id.') CACHE END 処理時間 = ' . $sec . ' 秒');
...
// 				return View_Smarty::forge( $tpl, $arrResult, false );
		...
		else
		{
			...
			try
			{
				$cache = hash('sha256', 'p'.$page.'o'.$orderby.'c'.$cat.'cate'.$category.'sub'.$subcategory.'co'.$color.'s'.$size.'b'.$brand.'sh'.$shopn.'shop'.$this->shop_url.'sl'.$sale.'v'.$view);

$end = microtime(true);
$sec = (float)((float)$end - (float)$start);
$this->debug(' CACHE END 処理時間 = ' . $sec . ' 秒');
				return Cache::get($cache_key.$cache);
			}
			catch(\CacheNotFoundException $e)
			{
				Cache::delete('item'.$cache);
			}
			catch(Exception $e)
			{
			}
...
// Profiler::console(count($arrResult['arrRecommend']));
		...
		Cache::set($cache_key.$cache,View_Smarty::forge( $tpl, $arrResult, false ),300);
$end = microtime(true);
$sec = (float)((float)$end - (float)$start);
if ($product_id != '')
	$this->debug(' ('.$product_id.') END 処理時間 = ' . $sec . ' 秒');
else
...
//		return Response::forge(Presenter::forge('home/404'), 404);
	...
	{	
		$fp = fopen("/var/www/bronline/fuel/app/logs/product".date('Ymd').".log", "a");
		if ($fp)
		{
