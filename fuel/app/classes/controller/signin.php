...
class Controller_Signin extends ControllerPage
{
	...
	public function action_index()
	{
		...
		if (Input::param('logout') == '1')
		{
			...
			setcookie("CUSTOMER_ID", "", time() - 3600, "/", "www.bronline.jp", 1);
		...
		if (isset($_COOKIE['CUSTOMER_ID']))
		{
			...
			$customer->customer->setLoginMemory('1');
			$customer->setSession();
	...
	public function post_index()
	{
		...
		if( $val->run()) {
			...
			if( $customer_id = $customer->chkPassword( $val->input('email'), $val->input('password'))) {
				...
				if ($val->input('login_memory') == '1')
				{
					setcookie("CUSTOMER_ID", $customer_id, time() + 86400*365, "/", "www.bronline.jp", 1);	//86400*365
//					exit;
					
//					$_COOKIE['CUSTOMER_ID'] = $customer_id;
				}
				else
				{
					setcookie("CUSTOMER_ID", "", time() - 3600, "/", "www.bronline.jp", 1);
