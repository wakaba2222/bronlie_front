...
//			$where = " WHERE ((A.status = 2 AND (A.view_date is NULL OR A.view_date <= '{$now}') AND (A.close_date is NULL OR A.close_date > '{$now}')) OR A.status = 3) AND A.product_id in ('{$product_id}')  and A.del_flg = 0 ";
			$where = " WHERE ((A.status = 2 AND (A.view_date is NULL OR A.view_date <= '{$now}') AND (A.close_date is NULL OR A.close_date > '{$now}' OR (A.close_date < '{$now}' AND A.reservation_flg = 1))) OR A.status = 3) AND A.product_id in ('{$product_id}')  and A.del_flg = 0 ";
