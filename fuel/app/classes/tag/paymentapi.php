...
class Tag_Paymentapi extends Model
{
    ...
    public function gmoGetMultiEntryParam( $inp, $type = '1' ) {
        ...
        $success_url = GMO_COMPLETE_URL;//Config::get('paymentapi.gmo_url_multi_entry');
        $error_url = GMO_ERROR_URL;//Config::get('paymentapi.gmo_url_multi_entry');
		...
		$merchant_id = Config::get('paymentapi.gmo_shop_id');
		$hash = Config::get('paymentapi.hash_code');
		$service_id = Config::get('paymentapi.service_id');

        $res = "";
        if ($type == 1)
	        $paymethod = 'credit3d2';//'credit';
	    else if ($type == 5)
	        $paymethod = 'rakutenv2';//'rakuten';
	    else
	    	$paymethod = "";

	    $res = $res.'<input type="hidden" name="pay_method" id="pay_method" value="'.$paymethod.'">';
	    $res = $res.'<input type="hidden" name="merchant_id" value="'.$merchant_id.'">';
	    $res = $res.'<input type="hidden" name="service_id" value="'.$service_id.'">';

		if ($member_id == '')
			$member_id = microtime();

	    $res = $res.'<input type="hidden" name="cust_code" value="'.$merchant_id.'-001-'.$member_id.'">';
	    $res = $res.'<input type="hidden" name="sps_cust_no" value="">';
	    $res = $res.'<input type="hidden" name="sps_payment_no" value="">';
	    $res = $res.'<input type="hidden" name="order_id" value="'.$order_id.'">';
	    $res = $res.'<input type="hidden" name="item_id" value="1">';

		$item_name2 = mb_convert_encoding($item_name,'SJIS');
	    $res = $res.'<input type="hidden" name="item_name" value="'.$item_name.'">';
	    $res = $res.'<input type="hidden" name="amount" value="'.$amount.'">';
	    $res = $res.'<input type="hidden" name="pay_type" value="0">';
	    $res = $res.'<input type="hidden" name="service_type" value="0">';
	    $res = $res.'<input type="hidden" name="terminal_type" value="0">';

	    $res = $res.'<input type="hidden" name="success_url" value="'.$success_url.'">';
	    $res = $res.'<input type="hidden" name="cancel_url" value="'.$cancel_url.'">';
	    $res = $res.'<input type="hidden" name="error_url" value="'.$error_url.'">';
	    $res = $res.'<input type="hidden" name="pagecon_url" value="'.$response_url.'">';

	    $res = $res.'<input type="hidden" name="request_date" value="'.$entry_date.'">';
	    $res = $res.'<input type="hidden" name="limit_second" value="600">';

		$checksum = $paymethod.$merchant_id.$service_id.$merchant_id.'-001-'.$member_id.$order_id."1".$item_name.$amount;
		$checksum .= "000".$success_url.$cancel_url.$error_url.$response_url.$entry_date."600".$hash;
		
//		$checksum = sha1(mb_convert_encoding($checksum,'UTF-8'));
		$checksum = sha1($checksum);

	    $res = $res.'<input type="hidden" name="sps_hashcode" value="'.$checksum.'">';
