...
function func_del(news_id) {
    res = confirm('このSTYLING情報を削除しても宜しいですか？');
    if(res == true) {
...
*}-->
        <tr>
            <th>クレジット</th>
            <td>
                <!--{if $arrErr.credit}--><span class="attention"><!--{$arrErr.credit}--></span><!--{/if}-->
                <textarea name="credit" cols="80" rows="20" wrap="soft" class="" maxlength="<!--{$smarty.const.LLTEXT_LEN}-->" style="background-color:<!--{if $arrErr.credit}--><!--{$smarty.const.ERR_COLOR|h}--><!--{/if}-->"><!--{"\n"}--><!--{$arrForm.credit|smarty:nodefaults}--></textarea><br />
...
*}-->
				...
				<a href="<!--{$smarty.const.HTTP_URL}-->mall/<!--{$arrNews[data].creator_id|h}-->/styling_entry/?entry=<!--{$arrNews[data].news_id}-->&admin=on" target="_blank"><!--{$arrNews[data].news_title|h|nl2br}--></a>
				<!--{else}-->
				<a href="<!--{$smarty.const.HTTP_URL}-->styling_entry/?entry=<!--{$arrNews[data].news_id}-->&admin=on" target="_blank"><!--{$arrNews[data].news_title|h|nl2br}--></a>
				<!--{/if}-->
            </td>
            <td>
