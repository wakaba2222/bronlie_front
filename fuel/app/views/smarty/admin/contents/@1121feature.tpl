...
function func_del(news_id) {
    res = confirm('このFEATURE情報を削除しても宜しいですか？');
    if(res == true) {
...
<!--{/foreach}-->
    ...
    <table>
        ...
        <tr>
            <th>PR</th>
            <td>
            	<label><input type="checkbox" name="pr" value="1" <!--{if $arrForm.pr == 1}-->checked<!--{/if}--> />PR</label>
            </td>
        </tr>
        <tr>
...
*}-->
        <tr>
            ...
            <td>
                ...
                <input type="text" name="news_title" size="60" class="box60"    value="<!--{$arrForm.news_title|h}-->" <!--{if $arrErr.news_title}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}--> maxlength="<!--{$smarty.const.MTEXT_LEN}-->" />
        ...
        <tr>
            <th>スマホ用タイトル<span class="attention"> *</span></th>
            <td>
                <!--{if $arrErr.sphone_title}--><span class="attention"><!--{$arrErr.sphone_title}--></span><!--{/if}-->
                <input type="text" name="sphone_title" size="60" class="box60"    value="<!--{$arrForm.sphone_title|h}-->" <!--{if $arrErr.sphone_title}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}--> maxlength="<!--{$smarty.const.MTEXT_LEN}-->" />
                <span class="attention"> (上限<!--{$smarty.const.MTEXT_LEN}-->文字)</span>
            </td>
        </tr>
        <tr>
...
<!--{*            <th>リスト用画像<br />[312×190]</th>*}-->
            <th>リスト用画像<br />[492×300]</th>
            <td>
                <a name="<!--{$key}-->"></a>
                <span class="attention"><!--{$arrErr[$key]}--></span>
                <!--{if $arrForm.arrFile[$key].filepath != ""}-->
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" name="articles" size="60" class="box60"    value="<!--{$arrForm.articles|h}-->" <!--{if $arrErr.articles}-->style="background-color:<!--{$smarty.const.ERR_COLOR|h}-->"<!--{/if}--> maxlength="<!--{$smarty.const.LLTEXT_LEN}-->" />
            </td>
        </tr>
        <tr>
    ...
    <table class="list">
        ...
        <tr style="background:<!--{if $arrNews[data].news_id != $tpl_news_id}-->#ffffff<!--{else}--><!--{$smarty.const.SELECT_RGB}--><!--{/if}-->;" class="center">
            ...
            <td><!--{$arrNews[data].creator_id|h}--></td>
...
*}-->
				...
				<a href="<!--{$smarty.const.HTTP_URL}-->mall/<!--{$arrNews[data].creator_id|h}-->/feature_entry/?entry=<!--{$arrNews[data].news_id}-->&admin=on" target="_blank"><!--{$arrNews[data].news_title|h|nl2br}--></a>
				<!--{else}-->
				<a href="<!--{$smarty.const.HTTP_URL}-->feature_entry/?entry=<!--{$arrNews[data].news_id}-->&admin=on" target="_blank"><!--{$arrNews[data].news_title|h|nl2br}--></a>
				<!--{/if}-->
            </td>
            <td>
