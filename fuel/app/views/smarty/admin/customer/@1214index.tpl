...
<input type="hidden" name="mode" value="search" />
    ...
    <table class="form">
        ...
        <tr>
            <th>会員メルマガ</th>
            <td colspan="3"><!--{html_checkboxes name="search_mailmaga" options=$arrMailMagazineType separator="&nbsp;" selected=$arrForm.search_mailmaga.value}--></td>
        </tr>
    </table>
    <div class="btn">
...
<!--{/foreach}-->
    ...
    <div class="btn">
        ...
        <!--{foreach from=$arrStatus item=ostatus key=ckey name=cc}-->
        <option value="<!--{$ckey}-->"><!--{$ostatus}--></option>
        <!--{/foreach}-->
        </select>
		<a class="btn-normal" href="javascript:;" onclick="fnModeSubmit('status_change','',''); return false;">会員状態変更</a>
    </div>
    <!--{if count($arrData) > 0}-->
    ...
    <table class="list" id="customer-search-result">
        ...
        <!--{foreach from=$arrData item=row}-->
            <tr>
            	<!--{assign var=cs value=`$row.status`}-->
                <td class="center" rowspan="2"><!--{$arrStatus[$cs]}--></td>
                ...
                <input type="checkbox" name="change_customer_id[]" value="<!--{$row.customer_id}-->" id="customer_id_<!--{$row.customer_id}-->"/><label for="customer_id_<!--{$row.customer_id}-->">一括変更</label></span>
				</td>
