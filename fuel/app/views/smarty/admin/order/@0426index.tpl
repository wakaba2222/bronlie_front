...
<!--{%/if%}-->
<!--{%if $smarty.SESSION.authority == 0%}-->
        <a class="btn-normal" href="javascript:;" onclick="fnFormModeSubmit('search_form', 'csv2','',''); return false;">CSV ダウンロード(受注毎)</a>
        <a class="btn-normal" href="javascript:;" onclick="fnFormModeSubmit('search_form', 'csv3','',''); return false;">CSV ダウンロード(B2専用)</a>
<!--{%/if%}-->
...
*%}-->
	<!--{%if $smarty.SESSION.authority == 0%}-->
        ...
        一括送信CSVを選択：<input type="file" name="number_csv" id="number_csv" onchange="number_check()" />
        <a class="btn-normal" href="javascript:;" id="number_action" onclick="fnNumberCsvSubmit(); return false;" style="display:none;"><span>発送一括送信(CSV)</span></a>
    </div>
    <!--{%/if%}-->
...
<!--{%/if%}-->
            ...
            <td class="right"><!--{%(Tag_Util::taxin_cal($arrResults[cnt].total-$arrResults[cnt].discount+$arrResults[cnt].fee+$arrResults[cnt].deliv_fee)-$arrResults[cnt].use_point)|number_format%}--></td>
