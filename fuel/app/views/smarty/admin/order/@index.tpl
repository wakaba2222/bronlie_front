    ...
    <table>
        <tr>
            ...
            <td>
                ...
                <label><input type="checkbox" name="search_send_status" value="1" <!--{%if $arrForm[$key].value == '1'%}-->checked<!--{%/if%}--> />発送済み非表示</label><br/>
            </td>
        </tr>
        <tr>
            <th>お名前</th>
...
<!--{%/foreach%}-->
    <h2>検索結果一覧</h2>
        ...
        <!--{%/if%}-->
        <a class="btn-normal" href="javascript:;" onclick="fnSelectCheckSubmit('/admin/pdf.php'); return false;"><span>PDF一括出力</span></a>
<!--{%*
        ...
        <option value="">選択してください</option>
        <!--{%foreach from=$arrORDERSTATUS item=ostatus key=ckey name=cc%}-->
        <!--{%if $ckey != 7 && $ckey != 3%}-->
        <option value="<!--{%$ckey%}-->"><!--{%$ostatus%}--></option>
        <!--{%/if%}-->
        <!--{%/foreach%}-->
        </select>
        <a class="btn-normal" href="javascript:;" onclick="fnModeSubmit('status_order', '', ''); return false;"><span>ステータス一括変更</span></a>
*%}-->
    </div>
    ...
    <div class="pager">
        ...
        <a <!--{%if $page == $smarty.section.pages.iteration%}-->class="active"<!--{%/if%}--> href="/admin/order?page=<!--{%$smarty.section.pages.iteration%}-->"><!--{%$smarty.section.pages.iteration%}--></a>
        <!--{%/section%}-->
    ...
    <!--{%* 検索結果表示テーブル *%}-->
        ...
        <tr>
            ...
            <th>全商品発送日</th>
            <th>対応状況
			<input type="checkbox" name="status_check" id="status_check" onclick="fnAllCheck(this, 'input[name=status_order_id[]]')" />
			</th>
...
*%}-->
        ...
        <tr style="background:<!--{%$arrORDERSTATUS_COLOR[$status]%}-->;">
            ...
            <td>
            <!--{%$arrResults[cnt].name01|h%}--> <!--{%$arrResults[cnt].name02|h%}-->
            </td>
            <td>
            ...
            <br><input type="checkbox" name="status_order_id[]" value="<!--{%$arrResults[cnt].order_id%}-->" id="status_order_id_<!--{%$arrResults[cnt].order_id%}-->"/><label for="status_order_id_<!--{%$arrResults[cnt].order_id%}-->">一括変更</label><br>
            <!--{%/if%}-->
			</td>
