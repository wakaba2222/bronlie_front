...
*%}-->
        ...
        <tr>
            ...
            <td>
                ...
                <input type="text" id="<!--{%$key%}-->" name="<!--{%$key%}-->" value="<!--{%$arrForm[$key].value%}-->" maxlength="30" size="30" class="box30" />
            </td>
        </tr>
        <tr>
            <th>部門</th>
...
*%}-->
        ...
        <tr>
            ...
            <span class="attention"><!--{%$arrErr[$key]%}--></span>
            <!--{%html_checkboxes name="$key" options=$arrSALESTATUS selected=$arrForm[$key].value%}-->
            </td>
        </tr>
        <tr>
            <th>お直し商品</th>
            <td colspan="3">
            <!--{%assign var=key value="search_payoff"%}-->
            <span class="attention"><!--{%$arrErr[$key]%}--></span>
            <!--{%html_checkboxes name="$key" options=$arrPayOff selected=$arrForm[$key].value%}-->
            </td>
        </tr>
        <tr>
            <th>予約商品</th>
            <td colspan="3">
            <!--{%assign var=key value="search_reservation"%}-->
            <span class="attention"><!--{%$arrErr[$key]%}--></span>
            <!--{%html_checkboxes name="$key" options=$arrResevation selected=$arrForm[$key].value%}-->
            </td>
        </tr>
    </table>
    <p style="color:red;">※セール区分、予約商品の2項目はAND条件での検索になります。</p>
    <div class="btn">
...
<!--        <a class="btn-tool" href="javascript:;" onclick="if (confirm('検索結果を一括削除します。よろしいですか？')) {fnChangeAction('/admin/product/del'); fnModeSubmit('del', '', ''); return false;} else {return false;}">一括削除</a>-->
        <!--{%if $smarty.session.stock_type == 3%}-->
        <a class="btn-tool" href="javascript:;" onclick="fnChangeAction('/admin/product/nextdownload'); fnModeSubmit('nextdownload', '', ''); return false;">NEXT Engine 対応 CSV ダウンロード</a>
        <!--{%/if%}-->
    ...
    <div class="pager">
		...
		<a <!--{%if $page == $smarty.section.cnt.iteration%}-->class="active"<!--{%/if%}--> href="/admin/product?page=<!--{%$smarty.section.cnt.iteration%}-->"><!--{%$smarty.section.cnt.iteration%}--></a>
		<!--{%/if%}-->
		<!--{%/section%}-->
    ...
    <!--{%if count($arrProducts) > 0%}-->
        ...
        <table class="list" id="products-search-result">
            ...
            <tr>
                ...
                <th rowspan="1">セール区分</th>
                <th nowrap>公開日/公開終了日</th>
                <th nowrap>カテゴリ</th>
                <th nowrap>ブランド</th>
                <th rowspan="1">商品画像</th>
                <th rowspan="1">グループコード</th>
...
*}-->
                    <td class="id" rowspan="1"><!--{%if $arrProducts[cnt].sale_status == 0%}-->通常商品<!--{%elseif $arrProducts[cnt].sale_status == 1%}-->シークレットセール対象<!--{%elseif $arrProducts[cnt].sale_status == 2%}-->VIPシークレットセール対象<!--{%/if%}--></td>
                    ...
                    <img src="/upload/images/<!--{%$arrProducts[cnt].login_id%}-->/<!--{%$arrProducts[cnt].path%}-->" width="65" height="65">
                    </td>
                    <td rowspan="1"><!--{%if array_key_exists('group_code', $arrProducts[cnt])%}--><!--{%$arrProducts[cnt].group_code%}--><!--{%/if%}--></td>
                    <td rowspan="1"><!--{%if array_key_exists('price01', $arrProducts[cnt])%}--><!--{%$arrProducts[cnt].price01|number_format%}--><!--{%/if%}--></td>
                    <td rowspan="1"><!--{%if array_key_exists('reservation_flg', $arrProducts[cnt])%}--><!--{%if $arrProducts[cnt].reservation_flg%}-->予約商品<!--{%else%}--><!--{%/if%}--><!--{%/if%}--></td>
                    <td rowspan="1"><!--{%if array_key_exists('pay_off', $arrProducts[cnt])%}--><!--{%if $arrProducts[cnt].pay_off%}-->お直し商品<!--{%else%}--><!--{%/if%}--><!--{%/if%}--></td>
                    ...
                    <td class="menu" rowspan="1"><span class="icon_confirm"><a href="<!--{%*$smarty.const.HTTP_URL|sfTrimURL*%}-->https://www.bronline.jp/mall/<!--{%$smarty.session.shop%}-->/item?detail=<!--{%$arrProducts[cnt].product_id%}-->" target="_blank">確認</a></span></td>
