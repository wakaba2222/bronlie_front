...
<!--{%*if $customer_email == 'hwakaba2222@gmail.com' || $customer_email == 'info@bronline.jp' || $customer_email == 'koizumi@brshop.jp'*%}-->
      <tr><td colspan="2" class="border_none"><hr></td></tr>
			<tr>
				<td>
  				<div class="wrap_radio">
...
<div id="recipent_area">
  ...
  <p class="repletion">代金引換の場合、弊社の領収書は発行できません。<br>詳細は<a href="https://www.bronline.jp/guide/payment/#receipt" target="_blank">こちら</a>をご確認ください</p>
...
$(function(){
	...
	$('.cupon_code').change(function(){
			var url = "/api/couponcheck.json";
			var _customer_id = '<!--{%$customer_id%}-->';
			var _coupon = $('.cupon_code').val();
	
			var data = {customer_id : _customer_id, coupon : _coupon};
			var res = sendApi(url, data, coupon_view);
	});
});
