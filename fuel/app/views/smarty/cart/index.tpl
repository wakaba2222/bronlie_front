...
<div id="guide_main" class="wrap_size" style="display: block;">
	...
	<div class="t-center" style="height: 100%;">
		<div class="areabox">
			<div class="wrap_cartinfo">
				<section class="tit_page"><h2 class="times">INFORMATION</h2></section>
				<h3>返品・交換と返金方法についてルール変更のお知らせ</h3>
				<p class="info_cart">
<br>
いつもB.R.ONLINEをご愛顧いただき、誠にありがとうございます。<br>
このたび、返品・交換と返金方法についてルール変更を実施することとなりました。<br>
<br>
・お客様都合での交換（サイズ・カラーなど）の受付を終了いたします。<br>
・返品時に最終ご注文金額が5,000円以下になった場合にいただいていた初回送料を無料化いたしました。<br>
・これまで銀行振込で行っていた返金につきましては、今後は株式会社DGフィナンシャルテクノロジーの提供する返金システム「CASHPOST」でご返金いたします。<br>
<br>
</p>
<p>
詳細は以下ショッピングガイドページでご確認ください。<br>
<!-- ▼ショッピングガイド「返品・交換」<br> -->
<a href="https://www.bronline.jp/guide/return/" target="_blank"><span style="text-decoration:underline;">https://www.bronline.jp/guide/return/</span></a><br>
</p>
			</div>

		<div class="btn_area" style="margin:40px auto;">
			<label><input type="checkbox" value="1" id="hide_view" style="appearance:auto;margin:0px auto 20px;vertical-align:middle;vertical-align: baseline;" />このウィンドウを今後表示させない</label>
			<a class="back_sys block close_btn" href="#">確認し買い物を続ける</a>
			<br>
		</div>
...
$(function(){	
	...
	$('#hide_view').click(function(){
		let date = new Date();
		date.setTime( date.getTime() + ( 24 * 60 * 60 * 1000 * 365 ));	//24 * 60 * 60 * 1000 * 365
		$.cookie('hide_view', '1', {expires: date, path:'/', domain:'www.bronline.jp', secure:true});
	});
	
//		$.removeCookie('hide_view');
//		$.removeCookie('next_view');
	var hide = $.cookie('hide_view');
	var next_hide = $.cookie('next_view');
//	alert(hide);
//	alert(next_hide);

<!--{%if $customer['customer_id']%}-->
//	alert(hide);
//		$.cookie('hide_view', '0', {path:'/', domain:'bronline3.wdemo.org', secure:true});
//		$.removeCookie('hide_view');
<!--{%else%}-->
hide = 0;
<!--{%/if%}-->
...
//		$.removeCookie('hide_view');
	...
	{
		$.cookie('next_view', '1', {path:'/', domain:'www.bronline.jp', secure:true});
	}
	else if (next_hide == '1')
	{
		$('#size_overlay').hide();
	...
	<div class="hidden t-center" style="height: 100%;">
		<div>
			<div class="wrap_cartinfo">
				...
				<h3>年末年始の営業について</h3>
				<p class="info_cart">
					日頃よりB.R.ONLINEをご愛顧頂き、誠にありがとうございます。<br>
					<br>
					下記の期間を年末年始休業とさせていただきます。<br>
					◼発送休業期間：2022年12月28日（水）〜2023年1月2日（月）<br>
					◼お問い合わせ対応休業期間：2022年12月28日（水）〜2023年1月2日（月）<br>
					<br>
					※お休み期間中もご注文は承っております。<br>
					<br>
					<span class="bold">12月27日（火）15時以降のご注文・お問い合わせ・返品交換のご対応に関しては<br>
					1月3日（火）より順次対応いたします。<br></span>
					<br>
