...
<tr <!--{%if ($total_price+330) > 300000%}-->style="display:none;"
	<!--{%/if%}-->><td colspan="2" class="border_none">
		<hr>
	</td>
</tr>
<tr <!--{%if ($total_price+330) > 300000%}-->style="display:none;"
...
<table class="option payment none-mb">
	<tbody>
		<tr>
			...
			<td class="payment_info">
				<p class="mb20">現在の有効ポイント：　<span class="bold">
						<!--{%$customer.point%}-->
					</span> ポイント</p>
...
<p class="repletion">お届け日・お届け時間はあくまで目安となっております。指定をいただいても交通事情等により遅れる場合もございます。
	<!--<br><span class="bold red">年末年始は配達日時指定をうけたまわることができません。ご不便をおかけし恐縮ですがご理解ご了承のほどお願い申し上げます。</span>-->
</p>
				...
				<td class="payment_info">
					...
					<div class="wrap_select">
						<select name="receipt_select" id="receipt_select" class="col1">
							<option value="" <!--{%if $receipt_tadashi|default:"" == ''%}-->selected
								<!--{%/if%}-->>選択してください
							</option>
							<option value="お品代として" <!--{%if $receipt_tadashi|default:"" == 'お品代として'%}-->selected
								<!--{%/if%}-->>お品代として
							</option>
							<option value="贈答品代として" <!--{%if $receipt_tadashi|default:"" == '贈答品代として'%}-->selected
								<!--{%/if%}-->>贈答品代として
							</option>
							<option value="衣装代として" <!--{%if $receipt_tadashi|default:"" == '衣装代として'%}-->selected
								<!--{%/if%}-->>衣装代として
							</option>
							<option value="サンプル代として" <!--{%if $receipt_tadashi|default:"" == 'サンプル代として'%}-->selected
								<!--{%/if%}-->>サンプル代として
							</option>
							<option value="その他" <!--{%if $receipt_tadashi|default:"" != 'サンプル代として' && $receipt_tadashi|default:"お品代として" != 'お品代として' && $receipt_tadashi|default:"お品代として" != '贈答品代として' && $receipt_tadashi|default:"お品代として" != '衣装代として'%}-->selected
								<!--{%/if%}-->>その他
							</option>
							<!-- suzuki mod end -->
						</select>
				...
				<td class="payment_info">
  				<div class="wrap_select">
						...
						<select name="simple_package" id="simple_package" class="col1">
							<!-- suzuki mod start value set&smartytag -->
							<option value="0" <!--{%$simple_package_0%}-->>紙袋を希望する</option>
							<option value="1" <!--{%$simple_package_1%}-->>ダンボール箱を希望する</option>
						</select>
					</div>
...
-->
  <p class="repletion"><span class="attention">ラッピングではございません。</span>商品お届け時、紙袋梱包かダンボール梱包のどちらかを選択してください。<br>
「紙袋を希望する」を選ばれた場合、事務局側で紙袋発送が可能と判断した商品を紙袋に梱包してお届けいたします。<br>ジャケットやバッグ、靴、香水等ワレ物など、簡易梱包ができないお品もありますのであらかじめご了承ください。
</p>
