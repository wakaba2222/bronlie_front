...
<!--{%foreach key=key from=$arrRecommend item=arrProduct%}-->
<!--{%if $sp%}-->
<!--{%if $key < 6%}-->
			<div>
...
<!--{%*
							<img class="lazyload" data-src="/upload/images/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
*%}-->
							<img class="lazyload" data-src="/upload/images/<!--{%$arrProduct.login_id%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
						</div>
				</div>
...
<!--{%/foreach%}-->
<!--{%*
			<a class="btn_more times sp_portrait_only" id="recommendsp_more_btn">LOAD MORE<i class="icon-arrow_down"></i></a>
			<p class="t-center loading sp_portrait_only" id="recommendsp_more_load"><img src="/common/images/ico/ajax-loader.gif" alt="loading"></p>
*%}-->
		</div>
	</div>
</section>
...
<script>
$(function(){
	$('.recommendsp_hidden').hide();
	$('#recommendsp_more_load').hide();
	var cnt = <!--{%$arrRecommend|count%}-->;
	if (cnt <= 6)
	{
		$('#recommendsp_more_btn').hide();
	}

	$('#recommendsp_more_btn').click(function(){
		$('#recommendsp_more_btn').hide();
		$('.recommendsp_hidden').show();
	});
});
</script>
<!--{%/if%}-->