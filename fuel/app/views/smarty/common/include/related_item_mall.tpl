<script src="https://cdn.jsdelivr.net/npm/lazyload@2.0.0-rc.2/lazyload.min.js"></script>
<script>
$(function(){
	lazyload();
});
</script>
<!--{%if $arrRelatedMall|count%}-->
<section class="sec_slide justin related">
	<div class="wrap_contents">
		<div class="head_01 tit_jp clearfix">
			<h2 class="">関連するアイテム / B.R.MALL</h2>
		</div>
		<div class="slide_justin related">
<!--{%foreach key=key from=$arrRelatedMall item=arrProduct%}-->
<!--{%if $sp%}-->
<!--{%if $key < 6%}-->
			<div>
			<p class="new mont_medi"><!--{%if $arrProduct.status_flg == '1'%}-->NEW<!--{%/if%}--></p>
			<p class="like_item <!--{%if $arrProduct.product_id|in_array:$arrWish%}--> active <!--{%/if%}-->" data-pid="<!--{%$arrProduct.product_id%}-->"></p>
<!--{%*
			<a class="block" href="/mall/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
*%}-->
			<a class="block" href="/mall/<!--{%$arrProduct.login_id%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
				<div class="wrap_thum_outer">
						<div class="wrap_thum_inner">
<!--{%*
							<img class="lazyload" data-src="/upload/images/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
*%}-->
							<img class="lazyload" data-src="/upload/images/<!--{%$arrProduct.login_id%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
						</div>
				</div>
				<div class="tit_justin">
					<p class="brand"><!--{%$arrProduct.brand_name%}--></p>
					<p class="category"><!--{%$arrProduct.name%}--></p>
						<p class="price">¥<!--{%number_format(Tag_Util::taxin_cal($arrProduct.price01))%}--></p>
					<p class="shop times"><!--{%$arrProduct.shop_name%}--></p>
				</div>
			</a>
			</div>
<!--{%elseif $key < 10%}-->
			<div class="mallsp_hidden">
			<p class="new mont_medi"><!--{%if $arrProduct.status_flg == '1'%}-->NEW<!--{%/if%}--></p>
			<p class="like_item <!--{%if $arrProduct.product_id|in_array:$arrWish%}--> active <!--{%/if%}-->" data-pid="<!--{%$arrProduct.product_id%}-->"></p>
<!--{%*
			<a class="block" href="/mall/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
*%}-->
			<a class="block" href="/mall/<!--{%$arrProduct.login_id%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
				<div class="wrap_thum_outer">
						<div class="wrap_thum_inner">
<!--{%*
							<img class="lazyload" data-src="/upload/images/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
*%}-->
							<img class="lazyload" data-src="/upload/images/<!--{%$arrProduct.login_id%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
						</div>
				</div>
				<div class="tit_justin">
					<p class="brand"><!--{%$arrProduct.brand_name%}--></p>
					<p class="category"><!--{%$arrProduct.name%}--></p>
						<p class="price">¥<!--{%number_format(Tag_Util::taxin_cal($arrProduct.price01))%}--></p>
					<p class="shop times"><!--{%$arrProduct.shop_name%}--></p>
				</div>
			</a>
			</div>
<!--{%/if%}-->

<!--{%else%}-->
<!--{%if $key < 10%}-->
			<div>
			<p class="new mont_medi"><!--{%if $arrProduct.status_flg == '1'%}-->NEW<!--{%/if%}--></p>
			<p class="like_item <!--{%if $arrProduct.product_id|in_array:$arrWish%}--> active <!--{%/if%}-->" data-pid="<!--{%$arrProduct.product_id%}-->"></p>
<!--{%*
			<a class="block" href="/mall/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
*%}-->
			<a class="block" href="/mall/<!--{%$arrProduct.login_id%}-->/item/?detail=<!--{%$arrProduct.product_id%}-->">
				<div class="wrap_thum_outer">
						<div class="wrap_thum_inner">
<!--{%*
							<img class="lazyload" data-src="/upload/images/<!--{%if $arrProduct.org_shop%}--><!--{%$arrProduct.org_shop%}--><!--{%else%}--><!--{%$arrProduct.login_id%}--><!--{%/if%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
*%}-->
							<img class="lazyload" data-src="/upload/images/<!--{%$arrProduct.login_id%}-->/<!--{%$arrProduct.path%}-->" alt="<!--{%$arrProduct.title%}-->"/>
						</div>
				</div>
				<div class="tit_justin">
					<p class="brand"><!--{%$arrProduct.brand_name%}--></p>
					<p class="category"><!--{%$arrProduct.name%}--></p>
						<p class="price">¥<!--{%number_format(Tag_Util::taxin_cal($arrProduct.price01))%}--></p>
					<p class="shop times"><!--{%$arrProduct.shop_name%}--></p>
				</div>
			</a>
			</div>
<!--{%/if%}-->
<!--{%/if%}-->
<!--{%/foreach%}-->
<!--{%*
			<a class="btn_more times sp_portrait_only" id="mallsp_more_btn">LOAD MORE<i class="icon-arrow_down"></i></a>
			<p class="t-center loading sp_portrait_only" id="mallsp_more_load"><img src="/common/images/ico/ajax-loader.gif" alt="loading"></p>
*%}-->
		</div>
	</div>
</section>
<!--{%/if%}-->
<!--{%if $sp%}-->
<script>
$(function(){
	$('.mallsp_hidden').hide();
	$('#mallsp_more_load').hide();
	var cnt = <!--{%$arrRelatedMall|count%}-->;
	if (cnt <= 6)
	{
		$('#mallsp_more_btn').hide();
	}

	$('#mallsp_more_btn').click(function(){
		$('#mallsp_more_btn').hide();
		$('.mallsp_hidden').show();
	});
});
</script>
<!--{%/if%}-->