...
<div class="wrap_contents sub">
	...
	<form name="form1" method="post" action="subscribe" >
		...
		<table class="form">
			...
			<!--{%/if%}-->
			<tr><th>希望パスワード <span class="red">※</span> <br class="sp_none">(半角英数)</th>
				<td><input type="password" placeholder="８桁以上の半角英数"class="col1 mb2 ph" name="password" value="<!--{%if array_key_exists('password', $arrForm) %}--><!--{%$arrForm['password']%}--><!--{%/if%}-->" >
			...
			<tr>
				...
				<td>
					...
					<input type="text" id="reminder_answer" class="col1 mb2 ph" name="reminder_answer" placeholder="質問の答え" value="<!--{%if array_key_exists('reminder_answer', $arrForm) %}--><!--{%$arrForm['reminder_answer']%}--><!--{%/if%}-->" >
				</td>
			</tr>
		...
		<button type="submit" class="submit_sys block">入力内容の確認へ進む</button>
	</form>
