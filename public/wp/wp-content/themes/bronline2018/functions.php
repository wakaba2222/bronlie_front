...
function get_br_posts( $params = array() ) {
	...
	*/
	...
	$sql .= " LEFT JOIN $wpdb->postmeta AS pm12 ON (p.ID = pm12.post_id AND pm12.meta_key = 'pin_on') ";
...
//var_dump($results);
	...
	{
		$temp['thumb_url'] = str_replace('origin', 'www', $temp['thumb_url']);
//		$temp['content'] = str_replace('origin', 'www', $temp['content']);
		$arrTemp[] = $temp;
	}
...
function get_br_users( $params = array() ) {
	...
	$sql .= "JOIN $wpdb->usermeta AS um0 ON (u.ID = um0.user_id AND um0.meta_key = 'wp_capabilities' ) ";
	...
	$sql .= " AND um0.meta_value not like '%only%' ";	// ユーザー権限IDに only を含まない
...
//	$sql .= " ORDER BY last_date DESC ";
	$sql .= " ORDER BY u2.post_date DESC ";
...
//var_dump($sql);
//	define("LOG_FILE","/var/www/bronline/fuel/app/logs/sql".date("YmdHis").".log");
//	$current = file_get_contents(LOG_FILE);
//	$current = date("Ymd H:i:s")."   ".$sql.PHP_EOL;
...
function get_blog_article_list( $per_page = 12, $page = 1, $shop_name = "", $user_id = "", $ids = array() ) {
	$params = array();
...
function get_br_post_detail( $post_id )
{
	global $wpdb;
...
function get_br_post( $post_id, $page = 1, $preview = false ) {
	...
	$sql .= " ,pm31.meta_value AS blog要素 ";
	$sql .= " ,pm32.meta_value AS product_view ";
	...
	$sql .= " LEFT JOIN $wpdb->postmeta AS pm31 ON (p.ID = pm31.post_id AND pm31.meta_key = 'blog要素') ";
	$sql .= " LEFT JOIN $wpdb->postmeta AS pm32 ON (p.ID = pm32.post_id AND pm32.meta_key = 'product_view') ";
	...
	$arrTemp = array();
	foreach($arrResult as $temp)
	{
		$temp['thumb_url'] = str_replace('origin', 'www', $temp['thumb_url']);
		$temp['main_image_url'] = str_replace('origin', 'www', $temp['main_image_url']);
		if(isset($temp['movie_url']))
			$temp['movie_url'] = str_replace('origin', 'www', $temp['movie_url']);
		$arrTemp[] = $temp;
	}
...
function get_style_snap( $post_id, $preview = false ) {
	...
	foreach($arrResult as $temp)
	{
		if (isset($temp['image1']))
			$temp['image1'] = str_replace('origin', 'www', $temp['image1']);
		if (isset($temp['image2']))
			$temp['image2'] = str_replace('origin', 'www', $temp['image2']);
		if (isset($temp['image3']))
			$temp['image3'] = str_replace('origin', 'www', $temp['image3']);
		if (isset($temp['image4']))
			$temp['image4'] = str_replace('origin', 'www', $temp['image4']);
		if (isset($temp['image5']))
			$temp['image5'] = str_replace('origin', 'www', $temp['image5']);
		if (isset($temp['image6']))
			$temp['image6'] = str_replace('origin', 'www', $temp['image6']);
		$arrTemp[] = $temp;
	}
...
//if (strpos($brand_name,'EGR') !== false)
//{
//print('<pre>');
//var_dump($sql);
//var_dump($arrBindValue);
//print('</pre>');
//}
